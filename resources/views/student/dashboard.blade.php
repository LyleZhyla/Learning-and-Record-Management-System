...
<div class="metric-grid student-dashboard-metrics">
    ...
    <article class="metric-card"><span class="metric-icon orange">✓</span><div><small>ASSESSMENTS SUBMITTED</small><strong>{{ $stats['assessments_completed'] }}/{{ $stats['assessments_total'] }}</strong><p>{{ $stats['assessments_remaining'] }} assessment{{ $stats['assessments_remaining'] === 1 ? '' : 's' }} remaining</p></div></article>
    <article class="metric-card"><span class="metric-icon violet">◎</span><div><small>COMPUTED GRADE</small><strong>{{ $stats['grade']===null?'—':number_format($stats['grade'],2) }}</strong><p>Based on graded work</p></div></article>
</div>
<section class="card student-dashboard-guide"><img class="snapie-character snapie-qr-character" src="{{ asset('images/characters/snapie-qr.webp') }}" alt="SNAPIE mascot holding a QR code"><div class="student-guide-content"><div class="card-heading"><div><h3>How QR attendance works</h3><p>Sign in to your Student account, scan the QR displayed by your facilitator, and the system records the correct status automatically.</p></div></div><div class="quick-action-grid student-dashboard-actions"><a href="{{ route('student.attendance.index') }}"><strong>Attendance history</strong><small>Review Present, Late, and Absent records.</small></a><a href="{{ route('student.materials.index') }}"><strong>Learning materials</strong><small>Open resources for your component and section.</small></a><a href="{{ route('student.grades.index') }}"><strong>Grade summary</strong><small>Track scores and weighted performance.</small></a></div></div></section>
@endsection
