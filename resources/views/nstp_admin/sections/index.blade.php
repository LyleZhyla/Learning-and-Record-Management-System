...
@section('content')
    ...
    <section class="card user-table-card">
        <form class="filter-bar" method="GET" action="{{ route($routePrefix.'.sections.index') }}">
            ...
            <button class="filter-button" type="submit">Apply filters</button>@if(request()->hasAny(['component','academic_year','semester','status']))<a class="clear-filter" href="{{ route($routePrefix.'.sections.index') }}">Clear</a>@endif
        </form>
