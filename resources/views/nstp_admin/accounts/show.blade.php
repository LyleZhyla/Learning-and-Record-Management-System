...
<section class="card user-table-card student-data-section"><div class="sectioning-toolbar"><div><span class="eyebrow">Current available student data</span><h3>Enrollment history</h3></div></div><div class="table-wrap"><table class="data-table"><thead><tr><th>Component</th><th>ROTC category</th><th>Approval</th><th>Shirt size</th><th>Section</th><th>Academic year</th><th>Semester</th><th>Status</th></tr></thead><tbody>@forelse($user->nstpEnrollments as $enrollment)<tr><td>{{ $enrollment->component?->code ?? '—' }}</td><td>{{ $enrollment->rotc_category ?? '—' }}</td><td>{{ $enrollment->rotc_approval_status ? ucfirst($enrollment->rotc_approval_status) : 'Not required' }}</td><td>{{ $enrollment->shirt_size ?? 'Not provided' }}</td><td>{{ $enrollment->section?->code ?? 'Unassigned' }}</td><td>{{ $enrollment->academic_year }}</td><td>{{ $enrollment->section?->semesterLabel() ?? ucfirst($enrollment->semester) }}</td><td>{{ str($enrollment->status)->headline() }}</td></tr>@empty<tr><td colspan="8" class="muted-cell">No enrollment records.</td></tr>@endforelse</tbody></table></div></section>
