...
<section class="card automated-sectioning-card sectioning-primary-card">
    <div class="automated-sectioning-copy">
        ...
        <h2>{{ (int) ($unsectionedCounts[$componentId] ?? 0) }} student(s) awaiting placement</h2>
        <p>Select the component and term. Assigned students will fill available seats first, and new sections will be created only when needed.</p>
    </div>
...
<section class="sectioning-sections-step">
    <div class="sectioning-list-heading sectioning-main-list-heading">
        <div><span class="eyebrow">Sections</span><strong>{{ $selectedComponent->code }} sections</strong><span>{{ $sections->count() }} total</span></div>
        <div class="sectioning-list-actions"><small>{{ $academicYear }} · {{ \App\Models\NstpSection::SEMESTERS[$semester] }}</small><a class="secondary-outline-button" href="{{ route($routePrefix.'.sections.create', ['component' => $componentId]) }}">+ Create section</a></div>
    </div>
    <div class="section-summary-grid">
        ...
        @empty
            <article class="section-summary-card empty-summary"><strong>No sections found</strong><span>Create a section manually or run automatic sectioning after assigning students from Student Accounts.</span></article>
        @endforelse
    </div>
</section>
@else
